    ...
    <div className="flex flex-col overflow-hidden size-full">
      ...
      <div className="flex-1 bg-gray-700 flex flex-col min-h-0 px-2 pb-2">
        <Tabs.Root value={tabValue} onValueChange={setTabValue}
                   className="flex-1 min-h-0 flex flex-col mt-2 text-white h-full">
          <Tabs.List className="flex flex-row flex-nowrap gap-3 flex-none border-b-gray-600 border-b-2">
            <Tabs.Trigger value="friends"
                          className={`px-2 py-1 hover-highlight rounded-t-md cursor-pointer ${tabValue === "friends" && "bg-white/8"}`}>Friends</Tabs.Trigger>
            <Tabs.Trigger value="pending"
                          className={`px-2 py-1 hover-highlight rounded-t-md cursor-pointer ${tabValue === "pending" && "bg-white/8"}`}>Pending</Tabs.Trigger>
            <Tabs.Trigger value="blocked"
                          className={`px-2 py-1 hover-highlight rounded-t-md cursor-pointer ${tabValue === "blocked" && "bg-white/8"}`}>Blocked</Tabs.Trigger>
            <Tabs.Trigger value="add-friend"
                          className={`px-2 py-1 hover-highlight rounded-t-md cursor-pointer ${tabValue === "add-friend" && "bg-white/8"}`}>Add
              Friend</Tabs.Trigger>
          </Tabs.List>
